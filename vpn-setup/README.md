...
ในการอนุญาตให้อุปกรณ์ลูกข่าย (Pi Zero 2 W) จากภายนอกเชื่อมต่อเข้ามายังเครือข่าย VPN ของสำนักงานใหญ่ (Pi 4 Server) จำเป็นต้องตั้งค่าดังนี้:
...
## 2. ⚙️ การตั้งค่า VPN Tunnel Health Check (ฝั่ง Pi Zero 2 W Client)
...
### สเต็ปที่ 2: รันตู้จำลอง PBX Simulator บน Pi Zero 2 W
บน Pi Zero 2 W ให้รัน Simulator เพื่อเปิดพอร์ต TCP `10001` รอรับสัญญาณควบคุมไฟฟ้า:
```bash
cd pbx-connector
...
บน Pi 4 ให้รันระบบ API Backend โดยกำหนดค่าสภาพแวดล้อมชี้ไปยัง IP VPN และพอร์ตของตู้จำลองบน Pi Z2W:
```bash
export PBX_MODE=tcp
...
2. **CommandQueue (Pi Z2W)** จะทำการรับแพ็คเก็ตคำสั่งส่งสัญญาณ ASCII `..ROOM101=1\r\n` (คำสั่งเปิดไฟห้อง 101) เข้าสู่คิวการทำงาน
3. **PBX Simulator (Pi Z2W)** ประมวลผลคำสั่ง ปรับสถานะห้อง 101 ในหน่วยความจำเป็น ON และส่งสัญญาณยืนยันการทำรายการสำเร็จตอบกลับไปหา Pi 4 ทันทีผ่านทาง TCP Socket ด้วยรูปแบบ ASCII `=>ROOM101=1\r\n`
4. **ผลลัพธ์ที่ตรวจเช็ค**:
   - บนหน้าจอ **PBX Simulator** (Pi Z2W): สถานะของห้อง 101 ต้องแสดงเป็น `ON`
